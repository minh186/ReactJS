...
function getId(){
	...
	if(isExistUser === false){
		alert('Không tìm thấy user có id trùng với số đã nhập!');
	}
}
console.log(getId());
...
function getEmail(){
	let kw = (prompt('Nhập keyword: '));
	let isExistUser = false;
	console.log('Email của các user có keyword "' + kw + '" là: ')
	for(let user of users){
		if(user.first_name.includes(kw) || user.last_name.includes(kw)){
			console.log(user.email);
			isExistUser = true;
		}
	}
	if(isExistUser === false){
		alert('Không tìm thấy user có keyword đã nhập!');
	}
}
getEmail();
...
function countByAge(){
	...
	if(count == 0){
		alert('Không có user nào có tuổi lớn hơn 50');
	}else{
		return count;
	}
}
let greaterThan50 = countByAge();
console.log('Số user có tuổi lớn hơn 50 là: ' + greaterThan50);



// Đếm số lượng user đã kết hôn và chưa kết hôn.
function countMarried(){
	let countIsMarried = 0;
	let countIsNotMarried = 0;
	for(let user of users){
	...
	console.log('Số lượng user đã kết hôn: ' + countIsMarried);
	console.log('Số lượng user chưa kết hôn: ' + countIsNotMarried);
}
countMarried();
...
function countByJob(){
	...
	for(let i=0; i<a.length; i++){
		...
		console.log('Số user thuộc ngành ' + a[i] + ' là: ' + count);
	}
}
countByJob();